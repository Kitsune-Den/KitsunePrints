...
import { SLOTS, DEFAULT_FRAME_PRESET_ID, DEFAULT_PACK_META, isDefaultPackMeta, type SlotDef, type SlotState, type PackMeta } from '../types/slots'
      ...
      <div className="max-w-6xl mx-auto px-6 py-10 pb-32">
        ...
        {isSlotTab && activeSlotTab ? (
          ...
          <section>
            ...
            {isDefaultPackMeta(meta) && (
              <div className="mb-6 max-w-3xl rounded-lg border border-orange-500/40 bg-orange-500/10 px-4 py-3 flex items-start gap-3">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-orange-400 flex-shrink-0 mt-0.5" aria-hidden="true">
                  <circle cx="12" cy="12" r="10" />
                  <line x1="12" y1="8" x2="12" y2="12" />
                  <line x1="12" y1="16" x2="12.01" y2="16" />
                </svg>
                <div>
                  <p className="text-sm text-orange-200 font-medium">Set your pack name + author before downloading</p>
                  <p className="text-xs text-orange-300/70 mt-1 leading-relaxed">
                    Default name &quot;My Picture Pack&quot; and an empty author would ship a generic-looking
                    modlet to anyone who installs it. Take a sec ~ this nag goes away once you've
                    given your pack a real name and credit.
                  </p>
                </div>
              </div>
            )}
      ...
      <div className="fixed bottom-0 inset-x-0 bg-zinc-950/95 backdrop-blur-md border-t border-zinc-800 z-40">
        <div className="max-w-6xl mx-auto px-6 py-3 flex flex-wrap items-center justify-between gap-4">
          ...
          <DownloadButton
            slots={slots}
            meta={meta}
            onRequestPackInfo={() => setActiveTab('pack-info')}
          />
        </div>
      </div>
    </>
