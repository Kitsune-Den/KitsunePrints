...
//
// Cache policy:
//   - /vanilla/* (8 reference atlases, ~12 MB total) and /frames/* (6 wood
//     preset PNGs, ~300 KB total) are content-immutable for any given
//     deploy: their filenames never change between releases. They're also
//     the dominant fetch cost on first paint of the picture-frame builder
//     tab. Cache them aggressively so repeat visits and page refreshes
//     never re-download the bytes. (Bug report context: HakurouHayate's
//     V2.6 b14 thread mentioned "takes a min for the textures to load" on
//     the frames page ~ this is the lever that fixes that for repeat
//     visits.) If we ever need to bust the cache for a specific atlas, the
//     content-hash filename pattern (e.g. _pictureFramed_atlas.v2.webp)
//     is the way ~ never edit a cached file in place.
...
app.use(express.static(path.join(__dirname, 'dist'), {
  setHeaders: (res, filePath) => {
    ...
    if (normalized.includes('/vanilla/') || normalized.includes('/frames/')) {
